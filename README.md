...
Начните с [полного актуального контекста](CONTEXT.md). [Передача контекста команде](docs/TEAM_HANDOFF.md) задаёт порядок работы, а [описание кейса](docs/replenishment-case.md) — сценарии и приёмку M1–M5. Расчёт объясняет каждую строку; решение и утверждение остаются за менеджером. Автоотправка заказа поставщику запрещена.
