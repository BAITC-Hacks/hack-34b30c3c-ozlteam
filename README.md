...
Сервис для менеджера закупа ТОО «Электрокомплект»: на основе истории продаж, остатков, товаров в пути и периодов отсутствия товара предлагает, сколько заказать у каждого поставщика. Расчёт объясняет каждую строку и не принимает закупочное решение за человека. Выбранный кейс и критерии приёмки зафиксированы в [описании задачи](docs/replenishment-case.md); общий путь товара и данных — в [схеме процесса](docs/replenishment-process.md); инструкция для команды и разбор материалов — в [передаче контекста](docs/TEAM_HANDOFF.md).

## Что реализовано

В проекте сохранены два сценария. Демонстрационная страница использует stateless
API `/replenishment/demo`, `/calculate`, `/import`, `/template`: её подтверждение
и CSV остаются локальными. Серверный сценарий `/replenishment/runs` сохраняет
историю расчётов, использует PostgreSQL/worker и связан с заказами `/orders` и
обменом `/integrations/1c`. Подключение этой страницы к серверным заказам ещё требуется.
...
- Список положительных рекомендаций по поставщикам с количеством, срочностью и обоснованием. В интерфейсе менеджер может изменить количество, исключить строку, просмотреть итог и подтвердить его перед выгрузкой CSV. Отправки поставщику нет.
- Синтетический демо-набор, импорт `.xlsx` по шаблону, API и автоматические тесты расчёта.

## Сценарий проверки за две минуты

1. Запустите проект по инструкции ниже, выполните `make seed` и откройте приложение. Войдите под локальным демонстрационным администратором из `.env.local` (значения создаются из `.env.local.example`; для входа нужен `SECURITY_JWT_SECRET`).
2. На странице «Пополнение склада» дождитесь расчёта синтетических данных. Откройте объяснение строки и проверьте отдельно остаток, входящую поставку, исключённые разовые продажи, stockout, сезонность и тренд.
3. Измените количество в одной строке либо установите `0`, затем откройте предпросмотр, подтвердите локально и скачайте CSV. Ни один заказ поставщику автоматически не уходит.
4. Скачайте заполненный «Шаблон XLSX», измените тестовые значения и загрузите файл обратно. API-контракт и пять проверок приёмки перечислены в [описании кейса](docs/replenishment-case.md).
...
Демо и XLSX-шаблон используют синтетические продажи и обезличенные ID. В переданных архивах `IEK` и `Systeme Electric` обнаружены таблицы продаж, остатков, MOQ и товаров в пути; полнота ассортимента требует подтверждения. В динамике продаж нет обезличенного ID клиента и цены; отсутствуют точные периоды stockout и справочник сроков поставки. Эти отчёты пока не являются прямым входом сервиса. [Карта книг и пробелов](docs/hackathon/tracklogic.md) поможет подготовить адаптер. Переданный пользователем пакет `TrackLogic/` хранится в закрытом командном репозитории; его коммерческие данные не предназначены для публичного размещения или внешних LLM. Строительный текстовый контекст в нём устарел. Реальная база 1С ещё не подключена: реализован нормализованный API обмена, для которого нужен адаптер конфигурации партнёра. Формат CSV также ещё не подтверждён как готовый импорт в 1С.
