...
Демо и XLSX-шаблон используют синтетические продажи и обезличенные ID. В переданных архивах `IEK` и `Systeme Electric` обнаружены таблицы продаж, остатков, MOQ и товаров в пути; маркировка склада `РЦ ЕКТ Рыскулова` указывает на связь с инфраструктурой партнёра, но полнота ассортимента требует подтверждения. В этих выгрузках нет обезличенного ID клиента, цены, точных периодов stockout и справочника сроков поставки. Поэтому они пока не являются прямым входом сервиса, а часть проверок проводится на синтетических данных. [Карта всех 12 книг и пробелов](docs/TEAM_HANDOFF.md#5-переданные-материалы-и-безопасная-работа-с-ними) поможет команде подготовить адаптер. Сырые архивы не включены в Git из-за транзакционных документов и коммерческих показателей. Текстовый файл о строительной AI-платформе является старой концепцией и не описывает выбранный кейс. Текущая версия не подключена к 1С и не отправляет заказы поставщикам; CSV — локальная выгрузка для дальнейшей работы в учётной системе. Формат CSV пока не подтверждён партнёром как готовый импорт в 1С.

Для основного расчёта не нужны OpenAI/NVIDIA API и внешняя сеть. В репозитории остаются общие инфраструктурные модули заготовки (пользователи, файлы, фоновые задачи, AI-провайдеры, Notes и бот), но они не участвуют в расчёте пополнения. Ключи и реальные клиентские данные нельзя коммитить в Git. Публичной развёрнутой версии пока нет: Compose ниже предназначен только для локальной разработки.

## Технологии и устройство

Frontend: React, TypeScript, Vite. Backend: FastAPI, Pydantic, `openpyxl` для импорта XLSX, `pytest` для проверок. Общий локальный стек также содержит PostgreSQL, Redis и worker; домен пополнения рассчитывает ответ в памяти и не сохраняет загруженную книгу в БД. Предметная логика находится в `backend/app/Domains/Replenishment/services`, разбор Excel — в `adapters`, HTTP API — в `controllers`; интерфейс — в `frontend/src/modules/replenishment`. Подробное описание каркаса и остальных модулей сохранено ниже.
