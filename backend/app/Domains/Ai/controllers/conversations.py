...
@router.post(
    ...
    "Для тестового заказа допустим выбор любых N товаров поставщика по 1 единице. "
    "Короткое согласие на предложение тестового заказа продолжает его подготовку; "
    "сохранение требует отдельного подтверждения карточки.",
)
