...
function ActivityPanel({
  ...
  if (section === "tasks") {
    return drafts.length ? <div className={styles.list}>
      ...
      {hasMoreTasks ? <button className={styles.moreLink} type="button" disabled={loadingMoreTasks} onClick={onLoadMoreTasks}>{loadingMoreTasks ? "Загружаем…" : "Показать ещё"}</button> : null}
      {moreError ? <p className={styles.footnote} role="alert">{moreError} Повторите загрузку.</p> : null}
      <Link className={styles.moreLink} to="/orders?status=draft" onClick={onNavigate}>Открыть список черновиков</Link>
...
export function RightRail() {
  ...
  async function loadMoreTasks() {
    ...
    try {
      ...
      if (!controller.signal.aborted) setMoreError(caught instanceof Error ? caught.message : "Не удалось загрузить черновики.");
    } finally {
